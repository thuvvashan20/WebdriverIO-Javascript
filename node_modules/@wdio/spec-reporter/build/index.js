...
exports.default = exports.DATA_TABLE_CONFIG = void 0;
...
exports.DATA_TABLE_CONFIG = DATA_TABLE_CONFIG;
...
class SpecReporter extends _reporter.default {
  ...
  getEventsToReport(suite) {
    return [...suite.tests, ...suite.hooks.filter(hook => Boolean(hook.error))];
  }
    ...
    for (const suite of suites) {
      ...
      for (const test of eventsToReport) {
        ...
        if (test.argument && test.argument.rows && test.argument.rows.length) {
          ...
          output.push(...(0, _table.table)(cells, DATA_TABLE_CONFIG).split('\n').filter(Boolean).map(line => `${testIndent}  ${line}`));
        }
      }
